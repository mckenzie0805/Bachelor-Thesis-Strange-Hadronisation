// ============================================================================
// Xi_pT_Spectra_Fresh.C - Fresh start for Xi pT spectra comparison
// ============================================================================
// Compares PYTHIA (Monash & Junctions) vs ALICE for Xi
// Three multiplicity selections: 0-1%, 0-20%, 80-100%
//
// ALICE mapping (from HEPData Table 52):
//   0-1%:     Table 21 (V0M Class I: 0.0-0.95%)
//   0-20%:    Tables 1-5 averaged (V0M Classes I-V: 0.0-19.0%)
//   80-100%:  Table 10 (V0M Class X: 68.0-100.0%)
//
// Usage:
// root -l 'Xi_pT_Spectra_Fresh.C("spectra_monash_100M.root", "spectra_junctions_100M.root", "HEPData-ins1471838-v1-root.root")'
//
...
TGraphErrors* GetPythiaSpectrum(TFile* pythiaFile, TH1D* hMult, int binLow, int binHigh, const char* name) {
    auto* h2D = dynamic_cast<TH2D*>(pythiaFile->Get("fHistPtXis"));
...
{
    std::cout << "\n╔══════════════════════════════════════════════════════╗\n";
    std::cout << "║  Xi pT Spectra: 0-1%, 0-20%, 80-100% Multiplicity  ║\n";
    std::cout << "╚══════════════════════════════════════════════════════╝\n\n";
    ...
    std::vector<MultSelection> selections = {
        {"0_1",   "0-1%",    binEdges[3], binEdges[4], {21}},           // Class I (highest mult)
        {"0_20",  "0-20%",   binEdges[2], binEdges[4], {21,22,23,24,25}},   // Classes I-V
        {"80_100","80-100%", binEdges[0], binEdges[1], {30}}           // Class X (lowest mult)
    };
    ...
    for (size_t i = 0; i < selections.size(); ++i) {
        ...
        TCanvas* c = new TCanvas(Form("c_%s", sel.name), Form("Xi %s", sel.label), 800, 700);
        c->SetLogy();
        c->SetGrid();
        ...
        // Set title with Xi and multiplicity label
        gMonash->SetTitle(Form("#Xi^{-} / #bar{#Xi}^{+} Transverse Momentum Spectrum (%s);p_{T} (GeV/c);(1/N_{ev}) d^{2}N/(dy dp_{T}) (GeV/c)^{-1}", sel.label));
        ...
        TString outName = Form("Xi_pT_Spectrum_%s.png", sel.name);
        c->SaveAs(outName);
